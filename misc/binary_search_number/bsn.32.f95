...
! Function (32-bit) to binary search a number in a sorted array

! ALL YOU NEED TO DO:
! Call function bsa_32(a, n1, n2, b) with:
...
! If the number does not exist in the array, -1 is returned
    ...
    n=size(aa)
    nba=nbsa_asc_32(aa,1,n,b)
    write(6,*) nba,aa(nba)
    nbd=nbsa_dec_32(ad,1,n,b)
    write(6,*) nbd,ad(nbd)
    end

! For ascendingly sorted array
    integer function nbsa_asc_32(a,nn1,nn2,b)
    real*8 a,b
    dimension a(nn2)
    if(b>a(nn2)) then
        nbsa_asc_32=-1  ! Not Found
        return
    endif
    if(b<a(nn1)) then
        nbsa_asc_32=-1  ! Not Found
...
! For descendingly sorted array
    integer function nbsa_dec_32(a,nn1,nn2,b)
    real*8 a,b
    dimension a(nn2)
    if(b<a(nn2)) then
        nbsa_dec_32=-1  ! Not Found
        return
    endif
    if(b>a(nn1)) then
        nbsa_dec_32=-1  ! Not Found
