...
! Subroutines (32-bit) to integrate a one-dimensional uniformly
...
! For Simpson's 1/3rd Rule, n should be odd
    ...
    call intd_trpzdl_32(a,b,f,n,s1)
    write(6,*) s1
    call intd_smpsn_32(a,b,f,n,s2)
    write(6,*) s2
    end

! Trapezoidal Rule Subroutine (32-bit)
! for 1D uniform data
    subroutine intd_trpzdl_32(a,b,f,n,sm)
    dimension f(n)  
    h=(b-a)/(n-1)
    sm=h*0.5*(f(1)+f(n))
    do i=2,n-1
...
! for 1D uniform data
    subroutine intd_smpsn_32(a,b,f,n,sm)
    dimension f(n)
    if(and(n,1)==0) then
    ...
    sm=(smi+4.0*smo+2.0*sme)*0.33333333
    end
