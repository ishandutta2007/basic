...
! Subroutines (64-bit) to integrate one-dimensional function
...
! Below is example of integration(d/(1+x*x),(x,0,1)), with d=4
    ...
    a=0.0d0; b=1.0d0; n=199
    d=4.0d0

    call int_trpzdl_64(fx,a,b,n,sm)
    write(6,*) sm
    call int_smpsn_64(fx,a,b,n,sm)
    write(6,*) sm
    end
...
! Truncation Error = -(b-a)(h^2)f''(z)/12, a<z<b
    subroutine int_trpzdl_64(f,a,b,n,s)
    implicit real*8(A-H,O-Z)
    external f
    h=(b-a)/(n-1)
    s=h*0.5d0*(f(a)+f(b))
...
! Truncation Error = -(b-a)(h^4)f''''(z)/180, a<z<b
    subroutine int_smpsn_64(f,a,b,n,s)
    implicit real*8(A-H,O-Z)
    external f
    if(and(n,1)==0) then
        m=n-1
    ...
    h=(b-a)/(m-1)
    s=h*(f(a)+f(b)+4.0d0*f(b-h))*0.333333333333333333d0
    do i=1,m/2-1
        s=s+h*(4.0d0*f(a+(2*i-1)*h)+2.0d0*f(a+2*i*h))*0.333333333333333333d0
    end do
    end
