...
! Subroutines (64-bit) to calculate single derivative of a
! one-dimensional uniformly spaced data using 5-Point Formula

! ALL YOU NEED TO DO:
! Call subroutine der1_64() with:
!   INPUT:
...
! Only first 10 values are printed on screen
    ...
    n=100
    do i=1,n
        f(i)=exp((i-1)*1.d0/(n-1))
    enddo

    call der1_64(n,1.d0/(n-1),f,df)

    do i=1,min(n,10)
        write(6,*) (i-1)*1.d0/(n-1),f(i),df(i)
    enddo

    end

! Subroutine (64-bit) to calculate single derivative of
! one-dimensional uniformly spaced data using
! the 5-point formula
! Time Complexity ~ O(n)
! Space Complexity ~ O(n)
! Truncation Error: h**4*f'''''(z)/30, x-2h<z<x+2h
    subroutine der1_64(n,h,f,df)
    implicit real*8(A-H,O-Z)
    dimension f(n),df(n)
    if(h==0.d0) then
    ...
    if(n>3) then
        h12=1.d0/(12.d0*h)
        a1=4.d0*f(1)-6.d0*f(2)+4.d0*f(3)-f(4)
        a2=10.d0*f(1)-20.d0*f(2)+15.d0*f(3)-4.d0*f(4)
        b1=4.d0*f(n)-6.d0*f(n-1)+4.d0*f(n-2)-f(n-3)
        b2=10.d0*f(n)-20.d0*f(n-1)+15.d0*f(n-2)-4.d0*f(n-3)
        df(1)=(a2-8.d0*a1+8.d0*f(2)-f(3))*h12
        df(n)=(f(n-2)-8.d0*f(n-1)+8.d0*b1-b2)*h12
        df(2)=(a1-8.d0*f(1)+8.d0*f(3)-f(4))*h12
        df(n-1)=(f(n-3)-8.d0*f(n-2)+8.d0*f(n)-b1)*h12
        do i=3,n-2
            df(i)=(f(i-2)-8.d0*f(i-1)+8.d0*f(i+1)-f(i+2))*h12
        enddo
    else if(n==3) then
        df(1)=-0.5d0*(3.d0*f(1)-4.d0*f(2)+f(3))/h
        df(2)=(f(3)-f(1))*0.5d0/h
        df(3)=0.5d0*(f(1)-4.d0*f(2)+3.d0*f(3))/h
    else if(n==2) then
        df(1)=(f(2)-f(1))/h
        df(2)=df(1)
    else
