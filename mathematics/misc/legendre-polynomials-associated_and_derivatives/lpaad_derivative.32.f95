...
! -1.0 <= x <= 1.0
    ...
    do l=0,4
        do m=-l,l
            call dplmfks32(l,m,x,pp)
            write(*,*) l,m,pp,dplmfkf32(l,m,x)
        enddo
    enddo
    end

! Function to calculate Derivative of Associated
! Legendre Polynomial of First Kind (32-bit)
    real function dplmfkf32(l,m1,x)
	real x,p(abs(m1):l),p0,p1,dn
    ifng=0
    if(m1<0) then
        ifng=1
        m=-m1
    ...
    if(ifng==0) then
        ...
        p0=p0*dn
        p1=p1*dn/(l+m)
        dplmfkf32=(p0-l*x*p1)/(1-x*x)
    endif
    endfunction

! Subroutine to calculate Derivative of Associated
! Legendre Polynomial of First Kind (32-bit)
    subroutine dplmfks32(l,m1,x,plm)
    integer,intent(in) :: l,m1
	real,intent(in) :: x
    real,intent(out) :: plm
    real p(abs(m1):l),dn,p0,p1
    ifng=0
    if(m1<0) then
        ifng=1
        m=-m1
