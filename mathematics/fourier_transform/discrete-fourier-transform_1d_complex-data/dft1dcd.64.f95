...
!   similar to numpy.fft.fftshift in Python
    ...
    ofl="dftc_ts02.txt"
	call dftc_1d_64(ifl,0,ofl)
    end

! Subroutine (64-bit) to calculate one-dimensional
! discrete fourier transform of uniformly spaced complex data
    subroutine dftc_1d_64(ifl,nh,ofl)
...
! To sinusoidally suppress initial and final nc points
	nc=-1
	do k=0,nc
		xc(k+1)=xc(k+1)*sin(k*pi*0.5d0/nc)
        xc(i-k)=xc(i-k)*sin(k*pi*0.5d0/nc)
    enddo

	ff=2.d0*pi/abs(t(i)-t(1))
    ft=(0.d0,0.d0)
    open(396,file=ofl)
    do j=(i-1)/2+1,i-1
    	do k=0,i-1
        	ft(j+1)=ft(j+1)+xc(k+1)*exp((0.d0,-2.d0)*pi*j*k/i)
        enddo
