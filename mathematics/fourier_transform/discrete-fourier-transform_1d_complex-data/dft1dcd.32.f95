...
!   similar to numpy.fft.fftshift in Python
    ...
    ofl="dftc_ts02.txt"
	call dftc_1d_32(ifl,0,ofl)
    end

! Subroutine (32-bit) to calculate one-dimensional
! discrete fourier transform of uniformly spaced complex data
    subroutine dftc_1d_32(ifl,nh,ofl)
    complex ft,xc
    character*150 ifl,ofl
...
! To sinusoidally suppress initial and final nc points
	nc=-1
	do k=0,nc
		xc(k+1)=xc(k+1)*sin(k*pi*0.5/nc)
        xc(i-k)=xc(i-k)*sin(k*pi*0.5/nc)
    enddo
    ...
    do j=(i-1)/2+1,i-1
    	do k=0,i-1
        	ft(j+1)=ft(j+1)+xc(k+1)*exp((0.,-2.)*pi*j*k/i)
        enddo
