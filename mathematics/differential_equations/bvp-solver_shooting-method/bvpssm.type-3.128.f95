...
! ALL YOU NEED TO DO:
! Call subroutine shooting3_128() with:
...
! INPUTS
    ...
    y11=1.q0; y22=2.1256q0      ! values for y(1) at x1 and y(2) at x2
    y2g1=-1.q0; y2g2=4.q0       ! guess values for y(2) at x1
    er=1.q-20                   ! tolerance for Bisection method
    
    ofln="sh3.out.txt"          ! output file name

    call shooting3_128(fx,nt,nf,x1,x2,y11,y22,y2g1,y2g2,er,ofln)
...
! Shooting Method BVP-3 (128-bit)
    subroutine shooting3_128(fx,nt,nf,x1,x2,y11,y22,y2g1,y2g2,er,ofln)
    implicit real*16(A-H,O-Z)
    character*150 ofln
    external fx
    ht=(x2-x1)/nt
    call bsctn3_128(fx,nt,ht,y11,y22,y2g1,y2g2,er,y21,istt)
    if(istt<0) then
...
! Bisection Method (128-bit) to be used by Shooting Method BVP-3
    subroutine bsctn3_128(fx,nt,ht,y11,y22,a0,b0,er,c,istt)
    implicit real*16(A-H,O-Z)
    logical ifa,ifb
    external fx
    fa=rk44bisection3_128(fx,ht,nt,x0,y11,a0)-y22
    fb=rk44bisection3_128(fx,ht,nt,x0,y11,b0)-y22
!    if(fa*fb>0.q0) write(6,*) "WARNING: Bisection Method may fail, consider choosing different guess values."
    ...
    do
        fc=rk44bisection3_128(fx,ht,nt,x0,y11,c)-y22
        if(abs(fc)<er) then
            istt=1
            exit
            ...
            if(ifa) then
                fa=rk44bisection3_128(fx,ht,nt,x0,y11,a)-y22
            else if(ifb) then
                fb=rk44bisection3_128(fx,ht,nt,x0,y11,b)-y22
            endif
            if(fa*fc<0.q0) then
                b=c
                ifa=.false.
...
! Runge Kutta 4 Method (128-bit) to be used by Bisection Method (SM BVP-3)
    real*16 function rk44bisection3_128(fx,h,ns,x0,y1,y2)
    implicit real*16(A-H,O-Z)
    dimension f(2),y(2)
