...
! equations with boundary values type-4 using Shooting Method
! via Bisection method and Runge Kutta 4 (RK4) method

! This program solves for two coupled ODEs:
!   y'(1)=f(x,y(1),y(2))(1)
!   y'(2)=f(x,y(1),y(2))(2)
!   y(2)=y21 at x=x1 and y(2)=y22 at x=x2

! ALL YOU NEED TO DO:
! Call subroutine shooting4_128() with:
...
!   y21, y22 = boundary values of y(2), i.e., y(2) at x1 and x2
...
! INPUTS
    ...
    y21=1.7635q0; y22=2.1256q0  ! values for y(2) at x1 and y(1) at x2
    y1g1=-1.q0; y1g2=3.q0       ! guess values for y(1) at x1
    er=1.q-20                   ! tolerance for Bisection method
    
    ofln="sh4.out.txt"          ! output file name

    call shooting4_128(fx,nt,nf,x1,x2,y21,y22,y1g1,y1g2,er,ofln)
...
! Shooting Method BVP-4 (128-bit)
    subroutine shooting4_128(fx,nt,nf,x1,x2,y21,y22,y1g1,y1g2,er,ofln)
    implicit real*16(A-H,O-Z)
    character*150 ofln
    external fx
    ht=(x2-x1)/nt
    call bsctn4_128(fx,nt,ht,y21,y22,y1g1,y1g2,er,y11,istt)
    if(istt<0) then
...
! Bisection Method (128-bit) to be used by Shooting Method BVP-4
    subroutine bsctn4_128(fx,nt,ht,y21,y22,a0,b0,er,c,istt)
    implicit real*16(A-H,O-Z)
    logical ifa,ifb
    external fx
    fa=rk44bisection4_128(fx,ht,nt,x0,a0,y21)-y22
    fb=rk44bisection4_128(fx,ht,nt,x0,b0,y21)-y22
!    if(fa*fb>0.q0) write(6,*) "WARNING: Bisection Method may fail, consider choosing different guess values."
    ...
    do
        fc=rk44bisection4_128(fx,ht,nt,x0,c,y21)-y22
        if(abs(fc)<er) then
            istt=1
            exit
            ...
            if(ifa) then
                fa=rk44bisection4_128(fx,ht,nt,x0,a,y21)-y22
            else if(ifb) then
                fb=rk44bisection4_128(fx,ht,nt,x0,b,y21)-y22
            endif
            if(fa*fc<0.q0) then
                b=c
                ifa=.false.
...
! Runge Kutta 4 Method (128-bit) to be used by Bisection Method (SM BVP-4)
    real*16 function rk44bisection4_128(fx,h,ns,x0,y1,y2)
    implicit real*16(A-H,O-Z)
    dimension f(2),y(2)
