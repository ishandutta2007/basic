...
! ALL YOU NEED TO DO:
! Call subroutine shooting2_64() with:
...
! INPUTS
    ...
    y21=1.7635d0; y12=3.d0      ! values for y(2) at x1 and y(1) at x2
    y1g1=-1.d0; y1g2=3.d0       ! guess values for y(1) at x1
    er=1.d-8                    ! tolerance for Bisection method
    
    ofln="sh2.out.txt"          ! output file name

    call shooting2_64(fx,nt,nf,x1,x2,y21,y12,y1g1,y1g2,er,ofln)
...
! Shooting Method BVP-2 (64-bit)
    subroutine shooting2_64(fx,nt,nf,x1,x2,y21,y12,y1g1,y1g2,er,ofln)
    implicit real*8(A-H,O-Z)
    character*150 ofln
    external fx
    ht=(x2-x1)/nt
    call bsctn2_64(fx,nt,ht,y21,y12,y1g1,y1g2,er,y11,istt)
    if(istt<0) then
...
! Bisection Method (64-bit) to be used by Shooting Method BVP-2
    subroutine bsctn2_64(fx,nt,ht,y21,y12,a0,b0,er,c,istt)
    implicit real*8(A-H,O-Z)
    logical ifa,ifb
    external fx
    fa=rk44bisection2_64(fx,ht,nt,x0,a0,y21)-y12
    fb=rk44bisection2_64(fx,ht,nt,x0,b0,y21)-y12
!    if(fa*fb>0.d0) write(6,*) "WARNING: Bisection Method may fail, consider choosing different guess values."
    ...
    do
        fc=rk44bisection2_64(fx,ht,nt,x0,c,y21)-y12
        if(abs(fc)<er) then
            istt=1
            exit
            ...
            if(ifa) then
                fa=rk44bisection2_64(fx,ht,nt,x0,a,y21)-y12
            else if(ifb) then
                fb=rk44bisection2_64(fx,ht,nt,x0,b,y21)-y12
            endif
            if(fa*fc<0) then
                b=c
                ifa=.false.
...
! Runge Kutta 4 Method (64-bit) to be used by Bisection Method (SM BVP-2)
    real*8 function rk44bisection2_64(fx,h,ns,x0,y1,y2)
    implicit real*8(A-H,O-Z)
    dimension f(2),y(2)
