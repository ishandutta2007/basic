...
% Function to solve Poisson's Equation in Two Dimensional Uniform
...
% Call the poisson solver function tspei2dugwdbcufdm5psaft(nx,x0,x1,ny,y0,y1), where
...
% similarly for u_yy; and f(x,y) is a user-input function
...
%   >> u=tspei2dugwdbcufdm5psaft(nx,x0,x1,ny,y0,y1);
...
function u=tspei2dugwdbcufdm5psaft(nx,x0,x1,ny,y0,y1)
    hx=(x1-x0)/(nx-1); hy=(y1-y0)/(ny-1);
    rhx2=1/hx/hx; rhy2=1/hy/hy;
    u=bcs(nx,x0,x1,ny,y0,y1);
    f=fxy(nx,x0,x1,ny,y0,y1);
    f(2:ny-1,2)=f(2:ny-1,2)-u(2:ny-1,1)*rhx2;
    f(2:ny-1,nx-1)=f(2:ny-1,nx-1)-u(2:ny-1,nx)*rhx2;
    f(2,2:nx-1)=f(2,2:nx-1)-u(1,2:nx-1)*rhy2;
    f(ny-1,2:nx-1)=f(ny-1,2:nx-1)-u(ny,2:nx-1)*rhy2;
