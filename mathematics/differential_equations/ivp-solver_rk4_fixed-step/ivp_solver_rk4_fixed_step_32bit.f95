...
! INPUTS
    ...
    ofln="rk4_32_tst.f95.txt"   ! output file name

    call rk4_32(gx,n,h,ns,x0,y0,ofln)
...
! RK4 subroutine (32-bit)
    subroutine rk4_32(fx,n,h,ns,x0,y0,ofln)
    ...
    do i=1,ns
        ...
        k1=h*f
        xh=x+0.5*h
        call fx(f,xh,y+0.5*k1,n)
        k2=h*f
        call fx(f,xh,y+0.5*k2,n)
        k3=h*f
         call fx(f,x+h,y+k3,n)
        k4=h*f
        x=x0+i*h
        y=y+(k1+2.0*(k2+k3)+k4)*0.166666666667
        write(479,*) x,y
    enddo
    close(479)
    endsubroutine
