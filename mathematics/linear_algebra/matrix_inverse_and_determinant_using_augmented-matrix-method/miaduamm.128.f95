...
! Subroutine (128-bit) to calculate the inverse and
! determinant of a matrix using Augmented Matrix Method

! The arguments of function invmat_128() are:
...
! ALL YOU NEED TO DO:
! Call subroutine invmat_128(n,a,ai,det)
...
!   small number like 1.q-10, then change four conditions marked by
...
!   But the subroutine is independent of this file reading
    ...
    call invmat_128(n,a(1:n,1:n),ai(1:n,1:n),det)
...
! Subroutine (128-bit) to calculate the inverse and determinant
! of a square matrix using Augmented Matrix Method
    subroutine invmat_128(n,aa,ai,det)
    implicit real*16(A-H,O-Z)
    logical exc
...
! Exchanging first row with anouther such that a[0]!=0
    if(a(1,1)==0.q0) then               ! zero-checking; abs(a(1,1))<eps
        do i=2,n
            if(a(i,1)/=0.q0) then       ! non-zero-checking; abs(a(i,1))>eps
                exc=.false.
                det=-1.q0*det
                do k=1,n
                    t1(k)=a(i,k)
...
!   and applying row exchanges as necessary
    do k=1,n-1
        if(a(k,k)==0.q0) then           ! zero-checking; abs(a(k,k))<eps
            exc=.true.
            do i=k+1,n
                if(a(i,k)/=0.q0) then   ! non-zero-checking; abs(a(i,k))>eps
                    exc=.false.
                    det=-1.q0*det
                    do j=1,n
                        t1(j)=a(i,j)
