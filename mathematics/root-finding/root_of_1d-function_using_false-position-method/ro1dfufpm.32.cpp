...
Function falspos_32() outputs the root
...
float f(float x);
float falspos_32(float a, float b, float er, int& istt);
...
// Function (32-bit) to find a root of f(x) using False Position Method
float falspos_32(float a, float b, float er, int& istt)
{
    float fa = f(a);
    if (fabs(fa)<er) {
