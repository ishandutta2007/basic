...
! ALL YOU NEED TO DO:
! Call subroutine falspos_128() with:
...
! Subroutine is optimized for minimum calls to f(x)
    ...
    call falspos_128(a0,b0,er,c,istt,f)
...
! Subroutine (128-bit) to find a root of f(x) using False Position Method
    subroutine falspos_128(a0,b0,er,c,istt,f)
    implicit real*16(A-H,O-Z)
    logical ifa
    external f
    fa=f(a0)
